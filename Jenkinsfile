node {
   ...
   {
       sh 'curl -v -X POST --data-binary @deploy/pumrpclientdeploy.yaml -H "Content-Type: application/x-yaml" vamp.vamp.marathon.mesos:23084/api/v1/deployments'
   }
   stage('Move GW 50/50')
   {
       input 'Do you approve deployment?'
       sh 'curl -v -X PUT --data-binary @deploy/updategw50.yaml -H "Content-Type: application/x-yaml" vamp.vamp.marathon.mesos:23084/api/v1/gateways/pumrpclient'
   }
   stage('Move Full')
   {
       input 'Do you approve deployment?'
       sh 'curl -v -X PUT --data-binary @deploy/updategw100.yaml -H "Content-Type: application/x-yaml" vamp.vamp.marathon.mesos:23084/api/v1/gateways/pumrpclient'
   }
   stage('Undeploy Previous App')
   {
       sh 'curl -v -X DELETE --data-binary @deploy/pumrpclientpredeploy.yaml -H "Content-Type: application/x-yaml" vamp.vamp.marathon.mesos:23084/api/v1/deployments/pumrpclient:$((${BUILD_ID}-1))'
